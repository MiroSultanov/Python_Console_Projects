...
def get_iris_boundaries(img, show=False):
    ...
    while (not ext_iris_circle and multiplier <= 0.7):
        multiplier += 0.05
        print
        'Searching exterior iris circle with multiplier ' + \
        str(multiplier)
        center_range = int(math.ceil(pupil_circle[2] * multiplier))
    ...
    if not ext_iris_circle:
        print
        'ERROR: Exterior iris circle not found!'
        return None, None
...
def find_ext_iris(img, pupil_circle, center_range, radius_range):
    ...
    if not total_circles:
        print
        "Running plan B on finding ext iris circle"
        param2 = 120
        while (param2 > 40 and len(total_circles) < 50):
            crt_circles = get_circles(
...
def filtered_circles(circles, draw=None):
    ...
    if not circles:
        print
        'Error: empty circles list in filtered_circles() !'
        return []
...
def getall_matches(rois_1, rois_2, dratio,
    ...
    for pos in ['right-side', 'left-side', 'bottom', 'complete']:
        if not rois_1[pos]['kp'] or not rois_2[pos]['kp']:
            print
            "KeyPoints not found in one of rois_x[pos]['kp'] !!!"
            print
            " -->", pos, len(rois_1[pos]['kp']), len(rois_2[pos]['kp'])
        else:
        ...
        if show:
            print
            "{0} matches: {1}".format(pos, str(len(matches)))
...
def get_matches(roipos_1, roipos_2,
    ...
    if not roipos_1['kp'] or not roipos_2['kp']:
        print
        "KeyPoints not found in one of roipos_x['kp'] !!!"
        return []
...
def standard_dev(x):
    if not x:
        print
        'Error: empty list parameter in standard_dev() !'
        print
